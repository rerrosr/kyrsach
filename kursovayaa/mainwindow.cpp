...
void MainWindow::updateUI()
{
    ...
    query.prepare("select Seat, Available from cinema_booking");
...
void MainWindow::on_btnReset_clicked()
{
    ...
    deleteQuery.prepare("DELETE FROM users");

    if (deleteQuery.exec()) {
        qDebug() << "All user records deleted successfully.";
    } else {
        qDebug() << "Error occurred while deleting user records: " << deleteQuery.lastError().text();
    }

    QSqlQuery query(MyDB::getInstance()->getDBInstance());
    query.prepare("update cinema_booking set Available = 'Y'");
